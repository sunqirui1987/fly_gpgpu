...
/** @enum ArgumentUploadType
  * Enum for upload type of kernel arguments. Specifies which compute API function should be used internally by Fly library to make
...
enum class ArgumentUploadType
{
    ...
    /** Argument will be located in local memory. Kernel arguments cannot be directly transferred into local memory from host memory.
      * Assigning local memory argument to kernel from Fly API simply means that the compute API will allocate enough local memory to hold number
